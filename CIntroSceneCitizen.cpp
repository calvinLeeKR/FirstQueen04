...
CIntroSceneCitizen::CIntroSceneCitizen()
{
	mIntroFile = CImageFile::New(MAKEINTRESOURCE(IDB_CITIZEN), L"IDB_CITIZEN");
...
bool CIntroSceneCitizen::isMustChanged()
{
	if (endFrameCount == CHANGE_SCENE_FRAME) return TRUE;
	else return false;
}

void CIntroSceneCitizen::onFrameMove()
{
...
void CIntroSceneCitizen::onDraw(HDC hdc)
{
	mBG.Draw(hdc);
