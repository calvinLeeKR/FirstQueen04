...
CIntroSceneSoulGirl::CIntroSceneSoulGirl()
{
	mIntroFile = CImageFile::New(MAKEINTRESOURCE(IDB_SOULGIRL), L"IDB_SOULGIRL");
...
bool CIntroSceneSoulGirl::isMustChanged()
{
	if (endFrameCount == CHANGE_SCENE_FRAME) return TRUE;
	else return false;
}

void CIntroSceneSoulGirl::onFrameMove()
{
...
void CIntroSceneSoulGirl::onDraw(HDC hdc)
{
	mBG.Draw(hdc);
