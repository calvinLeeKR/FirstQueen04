...
CIntroSceneCastle::CIntroSceneCastle()
{
	mIntroFile = CImageFile::New(MAKEINTRESOURCE(IDB_CASTLE), L"IDB_CASTLE");
...
bool CIntroSceneCastle::isMustChanged()
{
	if (endFrameCount == CHANGE_SCENE_FRAME) return TRUE;
	else return false;
}

void CIntroSceneCastle::onFrameMove()
{
...
void CIntroSceneCastle::onDraw(HDC hdc)
{
	mBG.Draw(hdc);
