...
CIntroSceneTitle::CIntroSceneTitle()
{
	mIntroFile = CImageFile::New(MAKEINTRESOURCE(IDB_TITLE), L"IDB_TITLE");
...
bool CIntroSceneTitle::isMustChanged()
{
	if (endFrameCount == CHANGE_SCENE_FRAME) return TRUE;
	else return false;
}

void CIntroSceneTitle::onFrameMove()
{
...
void CIntroSceneTitle::onDraw(HDC hdc)
{
	mBG.Draw(hdc);
