...
CIntroSceneKingHall::CIntroSceneKingHall()
{
	mIntroFile = CImageFile::New(MAKEINTRESOURCE(IDB_KINGHALL), L"IDB_KINGHALL");
...
bool CIntroSceneKingHall::isMustChanged()
{
	if (endFrameCount == CHANGE_SCENE_FRAME) return TRUE;
	else return false;
}

void CIntroSceneKingHall::onFrameMove()
{
...
void CIntroSceneKingHall::onDraw(HDC hdc)
{
	mBG.Draw(hdc);
