...
CIntroSceneWitch::CIntroSceneWitch()
{
	mIntroFile = CImageFile::New(MAKEINTRESOURCE(IDB_WITCH), L"IDB_WITCH");
...
bool CIntroSceneWitch::isMustChanged()
{
	if (endFrameCount == CHANGE_SCENE_FRAME) return TRUE;
	else return false;
}

void CIntroSceneWitch::onFrameMove()
{
...
void CIntroSceneWitch::onDraw(HDC hdc)
{
	mBG.Draw(hdc);
