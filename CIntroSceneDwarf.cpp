...
CIntroSceneDwarf::CIntroSceneDwarf()
{
	mIntroFile = CImageFile::New(MAKEINTRESOURCE(IDB_DWARF), L"IDB_DWARF");
...
bool CIntroSceneDwarf::isMustChanged()
{
	if (endFrameCount == CHANGE_SCENE_FRAME) return TRUE;
	else return false;
}

void CIntroSceneDwarf::onFrameMove()
{
...
void CIntroSceneDwarf::onDraw(HDC hdc)
{
	mBG.Draw(hdc);
