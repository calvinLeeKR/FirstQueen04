...
CIntroSceneKingOrb::CIntroSceneKingOrb()
{
	mIntroFile = CImageFile::New(MAKEINTRESOURCE(IDB_KINGORB), L"IDB_KINGORB");
...
bool CIntroSceneKingOrb::isMustChanged()
{
	if (endFrameCount == CHANGE_SCENE_FRAME) return TRUE;
	else return false;
}

void CIntroSceneKingOrb::onFrameMove()
{
...
void CIntroSceneKingOrb::onDraw(HDC hdc)
{
	mBG.Draw(hdc);
